...
ghenv.Component.Name = "Honeybee_HoneybeeCustom"
ghenv.Component.NickName = 'HoneybeeC'
...
if checkIn.letItFly:
    if not sc.sticky.has_key("honeybee_release") or True:
        ...
        ####################### userCustomBegin #######################
        """
        print folders.RADPath
        print folders.RADFile
        print folders.EPPath
        print folders.EPFile
        print folders.DSPath
        print folders.DSFile
        print folders.THERMPath
        print folders.THERMFile
        """
        hb_DefaultPFolder = ''
        if checkIn.folderIsSetByUser:
            hb_DefaultPFolder = "\\".join(sc.sticky["Honeybee_DefaultFolder"].split("\\")[:-2]) + "\\"
        ####################### userCustomEnd #######################
        ...
        if folders.EPPath == None:
            for EPVers in EPVersions:
                ...
                ####################### userCustomBegin #######################
                if checkIn.folderIsSetByUser:
                    if os.path.isdir(hb_DefaultPFolder + "EnergyPlus" + EPVers + "\\"):
                        folders.EPPath = hb_DefaultPFolder + "EnergyPlus" + EPVers + "\\"
                        EPVersion = EPVers
                ####################### userCustomEnd #######################
        ...
        installedOPS3 = []
        if checkIn.folderIsSetByUser:
            installedOPS3 = [f for f in os.listdir(hb_DefaultPFolder) if f.lower().startswith("openstudio")]
        ####################### userCustomEnd #######################
        try:
            ...
            ####################### userCustomBegin #######################
            if checkIn.folderIsSetByUser:
                installedOPS3 = sorted(installedOPS3, key=getversion, reverse=True)
            ####################### userCustomEnd #######################
        except Exception as e:
        ...
        if len(installedOPS2) != 0:
            ...
            if os.path.isdir(openStudioLibFolder) and os.path.isfile(os.path.join(openStudioLibFolder, "openStudio.dll")):
                ...
                QtFolder = None
        ####################### userCustomBegin #######################
        elif len(installedOPS3) != 0:
            if checkIn.folderIsSetByUser:
                installedOPS = installedOPS3[0]
                openStudioLibFolder = "%s%s\\CSharp\\openstudio"%(hb_DefaultPFolder, installedOPS)
                QtFolder = "%s%s\\Ruby\\"%(hb_DefaultPFolder, installedOPS)
                # Grab the version of EP that installs with OpenStudio
                if os.path.isdir("%s%s\\EnergyPlus\\"%(hb_DefaultPFolder, installedOPS)):
                    folders.EPPath = "%s%s\\EnergyPlus\\"%(hb_DefaultPFolder, installedOPS)
                    EPVersion = ""
                if os.path.isdir(openStudioLibFolder) and os.path.isfile(os.path.join(openStudioLibFolder, "OpenStudio.dll")):
                    # Add Openstudio to the path if it is not there already.
                    if not openStudioLibFolder in os.environ['PATH'] or QtFolder not in os.environ['PATH']:
                        os.environ['PATH'] = ";".join([openStudioLibFolder, QtFolder, os.environ['PATH']])
                    # Try to download the EP Run Files that are no longer distributed with OpenStudio but are necessary for Honeybee.
                    if not os.path.isfile('%s%s\\EnergyPlus\\Epl-run.bat'%(hb_DefaultPFolder, installedOPS)):
                        try:
                            client = System.Net.WebClient()
                            client.DownloadFile('https://github.com/mostaphaRoudsari/honeybee/raw/master/resources/EPRunFiles/Epl-run.bat', '%s%s/EnergyPlus/Epl-run.bat'%(hb_DefaultPFolder, installedOPS))
                        except Exception, e:
                            warning = 'Failed to download the files needed to run EnergyPlus with OpenStudio 2.x.'
                            warning += '\n' + `e`
                            print warning
                            ghenv.Component.AddRuntimeMessage(gh.GH_RuntimeMessageLevel.Warning, warning)
                    if not os.path.isdir('%s%s\\EnergyPlus\\PostProcess\\'%(hb_DefaultPFolder, installedOPS)):
                        try:
                            client = System.Net.WebClient()
                            client.DownloadFile('https://github.com/mostaphaRoudsari/honeybee/raw/master/resources/EPRunFiles/PostProcess.zip', '%s%s/EnergyPlus/PostProcess.zip'%(hb_DefaultPFolder, installedOPS))
                            sourceFile = '%s%s\\EnergyPlus\\PostProcess.zip'%(hb_DefaultPFolder, installedOPS)
                            with zipfile.ZipFile(sourceFile) as zf:
                                for member in zf.infolist():
                                    words = member.filename.split('\\')
                                    path = '%s%s\\EnergyPlus\\'%(hb_DefaultPFolder, installedOPS)
                                    for word in words[:-1]:
                                        drive, word = os.path.splitdrive(word)
                                        head, word = os.path.split(word)
                                        if word in (os.curdir, os.pardir, ''): continue
                            ...
                            print warning
                            ghenv.Component.AddRuntimeMessage(gh.GH_RuntimeMessageLevel.Warning, warning)
                    if not os.path.isdir('%s%s\\EnergyPlus\\PreProcess\\'%(hb_DefaultPFolder, installedOPS)):
                        os.mkdir('%s%s\\EnergyPlus\\PreProcess\\'%(hb_DefaultPFolder, installedOPS))
                else:
                    openStudioLibFolder = None
                    QtFolder = None
        ####################### userCustomEnd #######################
        ...
        if folders.RADPath == None:
            ...
            ####################### userCustomBegin #######################
            elif checkIn.folderIsSetByUser:
                if os.path.isdir("%s\\radiance\\bin\\"%hb_DefaultPFolder):
                    folders.RADPath = "%s\\radiance\\bin\\"%hb_DefaultPFolder
                else:
        ...
        if folders.DSPath == None:
            ...
            ####################### userCustomBegin #######################
                ...
                if os.path.isdir("%s\\daysim\\bin\\"%hb_DefaultPFolder):
                    folders.DSPath = "%s\\daysim\\bin\\"%hb_DefaultPFolder
                    print "Found installation of DAYSIM."
                else:
